...
use crate::{
    gamestate::{GameStateMachine, GameState, Action},
    picking::Picker,
    ui,
    scene::Scene,
    context::Context,
...
impl Engine {
    ...
    pub fn run(&mut self, initial_state: Box<dyn GameState>) {
        ...
        self.game_state_machine.push(&mut self.context, &mut self.scene, initial_state);

        self.picker.initialize_picking_attachments(&self.context.backend.display, (self.context.settings.graphics.window_width, self.context.settings.graphics.window_height));
        ...
        loop {
            match self.update() {
                ...
                Action::Push(gamestate) => {
                    self.game_state_machine.push(&self.context, &mut self.scene, gamestate);
                },
