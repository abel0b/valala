...
pub trait Stage<S,A> {
    ...
    fn frame(&mut self, ctx: &Context, scene: &mut Scene, store: &mut Store<S, A>) -> Transition<S,A>;
...
impl<S,A> StageMachine<S,A> {
    pub fn push(&mut self, ctx: &Context, mut stage: Box<dyn Stage<S,A>>) {
        if let Some(prevstate) = self.states.last_mut() {
            prevstate.pause(ctx);
        }

        let mut scene = Scene::default();
        stage.enter(ctx, &mut scene);
